...
void insert(Employee* node, Employee** root)
{
	...
	if (node->getFirstName().compare((*root)->getFirstName())<0)
	{
		insert(node, &((*root)->left));
	}
	else if(node->getFirstName().compare((*root)->getFirstName())>=0)
	{
...
Employee* depthFirstSearch(string firstName, Employee* root)
{
	...
	if (root->right != NULL)
	{
		temp = depthFirstSearch(firstName, root->right);
		if(temp != NULL)
			return temp; 
	}
		
	if(root->getFirstName().compare(firstName) == 0)
		return root; 
	
	if (root->left != NULL)
	{
		temp = depthFirstSearch(firstName, root->left);
		if(temp != NULL)
			return temp; 
	}
